...
public abstract class SpaceRenderer
{
	...
	public SpaceRenderer(long seed, int numberOfStars)
	{
	...
	protected void renderMoon(BufferBuilder bufferbuilder, Matrix4f lastMatrix, float size, int phase, float theta, float phi)
	{
		...
		this.createCelestialObject(bufferbuilder, lastMatrix, MOON_PHASES_LOCATION, 
				size, 100.0F, theta, phi, new float[] {xStart, yStart, xEnd, yEnd});
	...
	public void renderSky(ClientLevel level, float partialTicks, PoseStack stack, Camera camera, Matrix4f projectionMatrix, Runnable setupFog)
	{
		...
		this.renderSurface(bufferbuilder, stack.last().pose());
