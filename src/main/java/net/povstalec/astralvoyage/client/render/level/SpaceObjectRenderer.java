...
public final class SpaceObjectRenderer
{
	...
	private static final float INTERSTELLAR_MIN_SIZE = 0.5F;
	private static final float INTERSTELLAR_MAX_SIZE = 0.7F;
	private static final float SPACESHIP_MAX_SIZE = 0.25F;
	private static final float SPACESHIP_MIN_SIZE = 0.05F;
	...
	public static void renderSurface(BufferBuilder bufferbuilder, Matrix4f lastMatrix, ClientSpaceObject spaceObject, float distance, Vector3f galShipToObject, Vector3f shipToObject, float rotation)
	{
		...
		textureLayers.forEach(layer -> renderSurfaceLayer(bufferbuilder, lastMatrix, spaceObject.getSize(), distance, layer, shipToObject, galShipToObject, spaceObject.getOrbitOffset() == null ? 0D : spaceObject.getOrbitOffset(), rotation));
	}
	...
	{
		if(realSize == 1)
			return Math.min(Math.max((realSize/solarDistance)*1200, SPACESHIP_MIN_SIZE), SPACESHIP_MAX_SIZE);

		return Math.min(Math.max((realSize/solarDistance)*1200, galacticDistance > 0.1 ? INTERSTELLAR_MIN_SIZE : MIN_SIZE),
